...
__global__ void total(float * input, float * output, int len) {
	...
	__syncthreads();
	//below two lines are test to write output. Needs work to change.
	if (blockIdx.x*blockDim.x+t < len)
		output[t] = 0.0;
}
...
int main(int argc, char ** argv) {
	...
	dim3 dimGrid( (int)((numInputElements-1)/BLOCK_SIZE + 1), 
					   1,1);
	...
	wbCheck(cudaMemcpy(deviceOutput, deviceInput, numOutputElements * sizeof(float), cudaMemcpyDeviceToHost));
